    ...
    private static int Main(string[] args)
    {
        ...
        try
        {
            return command switch
            {
                ...
                "node" when args.Length >= 3 => NodeCommands.ParseNode(args[1], int.Parse(args[2])),
                "nodecompare" when args.Length >= 5 => NodeCommands.CompareNode(args[1], args[2], int.Parse(args[3]),
                    int.Parse(args[4])),
                "geometry" when args.Length >= 3 => GeometryCommands.Geometry(args[1], int.Parse(args[2])),
                "geomcompare" when args.Length >= 4 => GeometryCommands.GeomCompare(args[1], args[2],
                    int.Parse(args[3])),
                "vertices" when args.Length >= 3 => GeometryCommands.Vertices(args[1], int.Parse(args[2]),
                    args.Length > 3 ? int.Parse(args[3]) : 10),
                "colorcompare" when args.Length >= 5 => GeometryCommands.ColorCompare(args[1], args[2],
                    int.Parse(args[3]), int.Parse(args[4]), args.Length > 5 ? int.Parse(args[5]) : 20),
                "skinpart" when args.Length >= 3 => PackedCommands.SkinPart(args[1], int.Parse(args[2])),
                "skinpartcompare" when args.Length >= 5 => PackedCommands.SkinPartCompare(args[1], args[2],
                    int.Parse(args[3]), int.Parse(args[4]), args.Length > 5 ? int.Parse(args[5]) : 20),
                "packed" when args.Length >= 3 => PackedCommands.Packed(args[1], int.Parse(args[2]),
                    args.Length > 3 ? int.Parse(args[3]) : 5),
                "streamdump" when args.Length >= 3 => PackedCommands.StreamDump(args[1], int.Parse(args[2]),
                    args.Length > 3 ? int.Parse(args[3]) : 10),
                "analyzestreams" when args.Length >= 3 => PackedCommands.AnalyzeStreams(args[1], int.Parse(args[2]),
                    args.Length > 3 ? int.Parse(args[3]) : 5),
                "normalcompare" when args.Length >= 5 => PackedCommands.NormalCompare(args[1], args[2],
                    int.Parse(args[3]), int.Parse(args[4]), args.Length > 5 ? int.Parse(args[5]) : 50),
                ...
                "havokcompare" when args.Length >= 5 => HavokCommands.HavokCompare(args[1], args[2], int.Parse(args[3]),
                    int.Parse(args[4])),
                _ => PrintUsage()
            };
        }
    ...
    private static int PrintUsage()
    {
        Console.WriteLine("""
                          ...
                          Usage:
                            ...
                            NifAnalyzer colorcompare <f1> <f2> <b1> <b2> [count]
                                                                           Compare vertex colors between two blocks
                            NifAnalyzer skinpart <file> <block_index>      Parse NiSkinPartition block
                            NifAnalyzer skinpartcompare <f1> <f2> <b1> <b2> [count]
                                                                           Compare NiSkinPartition blocks side-by-side
                            NifAnalyzer packed <file> <block> [verts]      Parse BSPackedAdditionalGeometryData
                            NifAnalyzer analyzestreams <file> <block> [verts]
                                                                           Comprehensive stream analysis with semantics
                          ...
                          Examples:
                            ...
                            NifAnalyzer colorcompare converted.nif pc.nif 15 15 30
                            NifAnalyzer skinpart lefthand.nif 9
                            NifAnalyzer skinpartcompare converted.nif pc.nif 71 71 30
                            NifAnalyzer packed lefthand.nif 6 10
                            NifAnalyzer analyzestreams lefthand.nif 6 10
