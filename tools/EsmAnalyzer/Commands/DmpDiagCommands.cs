...
/// </summary>
public static class DmpDiagCommands
{
                ...
                if (noCell > 0)
                {
                    ...
                    var orphans = r.AllRefrs
                        .Where(x => !x.ParentCellFormId.HasValue && x.Position != null
                                    && (MathF.Abs(x.Position.X) > 1f || MathF.Abs(x.Position.Y) > 1f))
                        .ToList();
    ...
    internal static DmpResult ProcessDmp(string dmpFile, bool? forceProto = null)
    {
        ...
        return new DmpResult(
            FileName: fileName,
            TotalRecords: scanResult.MainRecords.Count,
            EsmRefrCount: esmRefrCount,
            RuntimeRefrFormEntries: scanResult.RuntimeRefrFormEntries.Count,
            RuntimeRefrRead: runtimeRefrRead,
            TotalRefrs: totalRefrs,
            PersistentCount: persistentRefs.Count,
            MarkerCount: markers.Count,
            CellCount: scanResult.MainRecords.Count(r => r.RecordType == "CELL"),
            WrldCount: scanResult.MainRecords.Count(r => r.RecordType == "WRLD"),
            LandCount: scanResult.MainRecords.Count(r => r.RecordType == "LAND"),
            Markers: markers,
