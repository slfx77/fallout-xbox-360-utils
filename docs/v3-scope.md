...
**Status:** Active. v2 complete; v3 work authorized. Open questions resolved (see "Resolved decisions" below). **Phase 0a complete 2026-05-29 — rendering pipeline migrated to Vortice.Windows; all 15 GPU tests green. Phase 0b complete 2026-05-29 — SwapChainPanel + Vortice swapchain wired through a 2D/3D toggle in the World Map tab, Sprite-mode removed, full build green, accessibility ratchet green, NpcRenderSmokeTests still green, spinning triangle visually confirmed (Path A validated — no Path B fallback needed). Phase 1 complete 2026-05-30 — cross-build cell-origin parity verified across all 4 builds (no per-build offsets needed), `CameraState` + `FlythroughCameraController` (WASD/Q/E + mouse-look + scroll-zoom), 6-plane `Frustum` extraction + AABB culling, `CellGridDebugRenderer` line-list wireframe; user confirmed cells render, camera controls, and HUD all work correctly. Phase 2a complete 2026-05-30 — heightmap-driven terrain meshes (preferring ESM `LandHeightmap` with `RuntimeTerrainMesh` fallback) uploaded lazily per cell via `TerrainRenderer`, flat alpha-blended water quads via `WaterRenderer` (cell `WaterHeight` → worldspace `DefaultWaterHeight` fallback, sentinel-aware), VCLR vertex tinting with simple Lambert lighting, D32 depth buffer added to `GpuSwapChainSurface`, layer toggles (D1/D2/D3) for wireframe/terrain/water; 6 new unit tests for `TerrainMeshBuilder` plus NpcRenderSmokeTests + accessibility ratchet all green.** Phase 2b (LTEX→TXST→texture resolver + multi-layer BTXT/ATXT/VTXT terrain shader) is next.
**Owner:** slfx77
