...
internal static class NifSkinPartitionParser
{
    ...
    public static ushort[]? ExtractVertexMap(byte[] data, int offset, int size, bool isBigEndian, bool verbose = false)
    {
        ...
        {
            var partitionStartPos = pos;

            // NumVertices (ushort)
            if (pos + 2 > end) { if (verbose) Console.WriteLine($"        Partition {p}: early break at NumVertices"); break; }
            ...
            // NumTriangles (ushort)
            if (pos + 2 > end) { if (verbose) Console.WriteLine($"        Partition {p}: early break at NumTriangles"); break; }
            ...
            // NumBones (ushort)
            if (pos + 2 > end) { if (verbose) Console.WriteLine($"        Partition {p}: early break at NumBones"); break; }
            ...
            // NumStrips (ushort)
            if (pos + 2 > end) { if (verbose) Console.WriteLine($"        Partition {p}: early break at NumStrips"); break; }
            ...
            if (pos + 2 > end) { if (verbose) Console.WriteLine($"        Partition {p}: early break at NumWeightsPerVertex"); break; }
            ...
            if (verbose)
                Console.WriteLine($"        Partition {p}: {numVertices} verts, {numTriangles} tris, {numBones} bones, {numStrips} strips, {numWeightsPerVertex} weights/vert");

            // Skip Bones array (numBones * ushort)
            pos += numBones * 2;
            if (pos > end) { if (verbose) Console.WriteLine($"        Partition {p}: early break after Bones"); break; }

            // HasVertexMap (byte)
            if (pos + 1 > end) { if (verbose) Console.WriteLine($"        Partition {p}: early break at HasVertexMap"); break; }
            ...
            if (hasVertexMap != 0)
            {
                if (pos + numVertices * 2 > end) { if (verbose) Console.WriteLine($"        Partition {p}: early break reading VertexMap"); break; }
