...
public static class EsmSemdiffCommand
{
        ...
        foreach (var sig in allSigs)
        {
            ...
            else
            {
                ...
                for (var i = 0; i < maxCount; i++)
                {
                    ...
                    if (subA == null)
                    {
                        diffs.Add(new FieldDiff(sig, null, subB!.Data, $"Only in B (index {i})", bigEndianA, bigEndianB,
                            recA.Type));
                    }
                    else if (subB == null)
                    {
                        diffs.Add(new FieldDiff(sig, subA.Data, null, $"Only in A (index {i})", bigEndianA, bigEndianB,
                            recA.Type));
                    }
    ...
    private sealed record RecordDiff(
        uint FormId,
        string RecordType,
        DiffType DiffType,
