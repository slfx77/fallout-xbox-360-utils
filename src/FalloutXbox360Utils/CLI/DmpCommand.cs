...
public static class DmpCommand
{
    ...
    private static Command CreateModulesCommand()
    {
        ...
        command.SetAction(parseResult =>
        {
            ...
            ListModules(input, format);
        });

        return command;
    }

    private static void ListModules(string path, string format)
    {
        ...
        AnsiConsole.MarkupLine($"[cyan]Build Type:[/] {buildType}");
    ...
    private static Command CreateRegionsCommand()
    {
        var inputArg = new Argument<string>("dump") { Description = "Path to the Xbox 360 minidump file" };
    ...
    private static void ListRegions(string path)
    {
        ...
        AnsiConsole.MarkupLine($"[cyan]Minidump:[/] {Path.GetFileName(path)}");
        AnsiConsole.MarkupLine(
            $"[cyan]Architecture:[/] {(info.IsXbox360 ? "Xbox 360 (PowerPC)" : $"0x{info.ProcessorArchitecture:X4}")}");
    ...
    private static Command CreateVa2OffsetCommand()
    {
        var inputArg = new Argument<string>("dump") { Description = "Path to the Xbox 360 minidump file" };
    ...
    private static void ConvertVaToOffset(string path, string vaStr)
    {
        ...
        {
            AnsiConsole.MarkupLine(
                $"[dim]Module: {Path.GetFileName(module.Name)} (base 0x{module.BaseAddress32:X8})[/]");
        }
    }

    // ===== hexdump =====

    private static Command CreateHexDumpCommand()
    {
        var inputArg = new Argument<string>("dump") { Description = "Path to the Xbox 360 minidump file" };
        var addressArg = new Argument<string>("address")
        {
            Description = "Address (hex). Prefix with 'va:' for virtual address, otherwise file offset"
        };
        var lengthOpt = new Option<int>("-n", "--length")
        {
