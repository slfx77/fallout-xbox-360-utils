...
/// </summary>
public static class EsmConvertCommand
{
    /// <summary>
    ...
    private static void PrintFallbackUsage()
    {
        ...
        foreach (var (fallbackType, recordType, subrecord, dataLength, count) in SubrecordSchemaRegistry
                     .GetFallbackUsage())
        {
            table.AddRow(
                $"[yellow]{fallbackType}[/]",
        ...
        foreach (var (type, totalCount, uniqueSubrecords) in summary)
        {
            AnsiConsole.MarkupLine(
                $"  [yellow]{type}[/]: {totalCount:N0} occurrences across {uniqueSubrecords} unique subrecord(s)");
        }
    }
}
