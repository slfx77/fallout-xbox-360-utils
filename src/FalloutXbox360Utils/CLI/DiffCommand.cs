        ...
        try
        {
            ...
            using (resultB)
            {
                ...
                if (onlyInA.Count > 0)
                {
                    ...
                    foreach (var r in onlyInA.Take(limit))
                    {
                        table.AddRow(
                            $"0x{r.FormId:X8}",
                            Markup.Escape(r.Type),
                            Markup.Escape(r.EditorId ?? ""),
                            Markup.Escape(r.DisplayName ?? ""));
                        shown++;
                    }

                    AnsiConsole.Write(table);
                    if (onlyInA.Count > limit)
                    {
                        AnsiConsole.MarkupLine($"[grey]... {onlyInA.Count - limit} more[/]");
                    }
                }
                ...
                if (onlyInB.Count > 0)
                {
                    ...
                    foreach (var r in onlyInB.Take(limit))
                    {
                        table.AddRow(
                            $"0x{r.FormId:X8}",
                            Markup.Escape(r.Type),
                    ...
                    AnsiConsole.Write(table);
                    if (onlyInB.Count > limit)
                    {
                        AnsiConsole.MarkupLine($"[grey]... {onlyInB.Count - limit} more[/]");
                    }
                }
