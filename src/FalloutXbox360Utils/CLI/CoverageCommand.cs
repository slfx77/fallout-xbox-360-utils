    ...
    private static async Task ExecuteAsync(string input, string? output, bool verbose, string? pdbPath = null)
    {
        ...
        if (!string.IsNullOrEmpty(output))
        {
            ...
            var reportText = CliHelpers.CaptureSpectreOutput(console =>
                RenderToConsole(console, coverage, Path.GetFileName(input), verbose));
            await File.WriteAllTextAsync(output, reportText);
            AnsiConsole.MarkupLine($"[green]Report saved to:[/] {output}");
        }
        else
        {
            RenderToConsole(AnsiConsole.Console, coverage, Path.GetFileName(input), verbose);
        }
    }

    private static void RenderToConsole(
        IAnsiConsole console, CoverageResult coverage, string fileName, bool verbose)
    {
        // Section 1: Overview
        ...
        console.Write(classificationPanel);
        console.WriteLine();
        ...
        if (gapLimit > 0)
        {
            ...
            for (var i = 0; i < gapLimit; i++)
            {
                ...
                table.AddRow(
                    ...
                    va,
                    CliHelpers.FormatSize(gap.Size),
        ...
        {
            console.WriteLine();
            RenderPdbAnalysisToConsole(console, coverage.PdbAnalysis);
        }
    }

    private static void RenderPdbAnalysisToConsole(IAnsiConsole console, PdbAnalysisResult pdb)
    {
