...
internal static class EsmDiffHeaderCommand
{
    public static int DiffHeader(string fileAPath, string fileBPath, string labelA = "Xbox 360", string labelB = "PC")
    {
        ...
        // FormID (4 bytes)
        var formIdA = bigEndianA
            ? BinaryUtils.ReadUInt32BE(dataA.AsSpan(12))
            : BinaryUtils.ReadUInt32LE(dataA.AsSpan(12));
        var formIdB = bigEndianB
            ? BinaryUtils.ReadUInt32BE(dataB.AsSpan(12))
            : BinaryUtils.ReadUInt32LE(dataB.AsSpan(12));
        table.AddRow(
            "0x0C", "FormID", "4",
        ...
        var hedrSizeA = bigEndianA
            ? BinaryUtils.ReadUInt16BE(dataA.AsSpan(hedrOffset + 4))
            : BinaryUtils.ReadUInt16LE(dataA.AsSpan(hedrOffset + 4));
        var hedrSizeB = bigEndianB
            ? BinaryUtils.ReadUInt16BE(dataB.AsSpan(hedrOffset + 4))
            : BinaryUtils.ReadUInt16LE(dataB.AsSpan(hedrOffset + 4));
        hedrTable.AddRow(
            "0x1C", "Size", "2",
        ...
        var numRecA = bigEndianA
            ? BinaryUtils.ReadUInt32BE(dataA.AsSpan(hedrDataOffset + 4))
            : BinaryUtils.ReadUInt32LE(dataA.AsSpan(hedrDataOffset + 4));
        var numRecB = bigEndianB
            ? BinaryUtils.ReadUInt32BE(dataB.AsSpan(hedrDataOffset + 4))
            : BinaryUtils.ReadUInt32LE(dataB.AsSpan(hedrDataOffset + 4));
        hedrTable.AddRow(
            "0x22", "NumRecords", "4",
        ...
        var nextIdA = bigEndianA
            ? BinaryUtils.ReadUInt32BE(dataA.AsSpan(hedrDataOffset + 8))
            : BinaryUtils.ReadUInt32LE(dataA.AsSpan(hedrDataOffset + 8));
        var nextIdB = bigEndianB
            ? BinaryUtils.ReadUInt32BE(dataB.AsSpan(hedrDataOffset + 8))
            : BinaryUtils.ReadUInt32LE(dataB.AsSpan(hedrDataOffset + 8));
        hedrTable.AddRow(
            "0x26", "NextObjectId", "4",
