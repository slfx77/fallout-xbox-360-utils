...
// Skeleton-based overloads (those that take CrossDumpSourceProjection / *Skeleton)
// mirror the RecordCollection-based builders for the streaming-pipeline. The
// originating PlacedReference is still passed through to NpcPlacementInfo /
// KeyLockedDoorInfo / ContainerPlacementInfo via PlacedObjectSkeleton.Ref, so the
// downstream Info shape doesn't change between the two pipelines.
