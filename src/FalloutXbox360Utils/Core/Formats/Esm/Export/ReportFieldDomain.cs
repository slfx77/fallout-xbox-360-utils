...
///         <item>Section-wildcard rule (<c>(RecordType, sectionGlob)</c>, covers repeating
///             structures like <c>Contents (0)</c> / <c>Contents (1)</c>).</item>
///         <item>Field-suffix family — the last whitespace-separated token of the field
///             name maps to a family rule (<c>*Path</c>→string, <c>*Count</c>→int≥0, etc.).</item>
///         <item>Sample-shape classifier — regex on the value's pre-formatted display
///             string infers the expected shape (int/float/hex/bool/path/resolved-formid).</item>
///     </list>
...
internal static class ReportFieldDomain
{
    ...
    private static Dictionary<(string Record, string Section, string Field), IDomainRule> BuildRules()
    {
        ...
        {
            rules[(t, "Identity", "FormID")] = new StringNonEmpty(MaxLen: 12);
            rules[(t, "Identity", "Editor ID")] = new StringNonEmpty(MaxLen: 256);
            rules[(t, "Identity", "Display Name")] = new StringNonEmpty(MaxLen: 1024);
        }
    ...
    private static Dictionary<string, IDomainRule> BuildSuffixFamily()
    {
        ...
        foreach (var s in new[] { "path", "model", "icon", "texture", "nif" })
            f[s] = new StringNonEmpty(MaxLen: 4096);
    ...
    private static List<(Regex Pattern, IDomainRule Rule)> BuildSampleShapeRules()
    {
        ...
        [
            ...
            (new Regex(@"[\\/].+\.(nif|dds|ddx|bsa|wav|xwm|xma|mp3|lip|kf|ini|txt|esp|esm|bmp|jpg|png)\b",
                RegexOptions.IgnoreCase | opts), new StringNonEmpty(MaxLen: 4096)),
