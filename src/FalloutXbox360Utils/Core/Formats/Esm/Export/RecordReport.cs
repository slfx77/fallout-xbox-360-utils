...
internal abstract record ReportValue
{
    ...
    internal sealed record IntVal(int Raw) : ReportValue
    {
        ...
        internal IntVal(int raw, string display) : this(raw) => _display = display;
    ...
    {
        private readonly string? _display;
        internal FloatVal(double raw, string display) : this(raw) => _display = display;
    ...
    {
        private readonly string? _display;
        internal BoolVal(bool raw, string display) : this(raw) => _display = display;
    ...
    {
        private readonly string? _display;
        internal FormIdVal(uint raw, string display) : this(raw) => _display = display;
    ...
    {
        private readonly string? _display;
        internal ListVal(List<ReportValue> items, string display) : this(items) => _display = display;
    ...
    {
        private readonly string? _display;
        internal CompositeVal(List<ReportField> fields, string display) : this(fields) => _display = display;
        public override string Display => _display ?? "(composite)";
    }

    // --- Factory helpers for concise construction ---

    public static IntVal Int(int value) => new(value);
    public static IntVal Int(int value, string display) => new(value, display);
    public static FloatVal Float(double value, string format = "F1") => new(value, value.ToString(format));
    public static FloatVal FloatDisplay(double value, string display) => new(value, display);
    public static StringVal String(string value) => new(value);
    public static BoolVal Bool(bool value) => new(value);
    public static BoolVal Bool(bool value, string display) => new(value, display);

    public static FormIdVal FormId(uint raw, string display) => new(raw, display);

    public static FormIdVal FormId(uint raw, FormIdResolver resolver) =>
        new(raw, resolver.FormatFull(raw));

    public static ListVal List(List<ReportValue> items) => new(items);
