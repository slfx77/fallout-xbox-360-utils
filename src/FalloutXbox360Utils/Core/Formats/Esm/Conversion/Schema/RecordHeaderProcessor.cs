...
public static class RecordHeaderProcessor
{
    ...
    public static MainRecordHeader ReadRecordHeader(ReadOnlySpan<byte> data, int offset = 0)
    {
        ...
        {
            Signature = signature,
            DataSize = BinaryPrimitives.ReadUInt32BigEndian(span[RecordHeaderSchema.RecordDataSizeOffset..]),
            Flags = BinaryPrimitives.ReadUInt32BigEndian(span[RecordHeaderSchema.RecordFlagsOffset..]),
    ...
    public static GroupHeader ReadGrupHeader(ReadOnlySpan<byte> data, int offset = 0)
    {
        var span = data.Slice(offset, RecordHeaderSchema.GrupHeaderSize);
        var label = BinaryPrimitives.ReadUInt32BigEndian(span[RecordHeaderSchema.GrupLabelOffset..]);

        return new GroupHeader
        {
            GroupSize = BinaryPrimitives.ReadUInt32BigEndian(span[RecordHeaderSchema.GrupSizeOffset..]),
            Label = BitConverter.GetBytes(label),
            GroupType = (int)BinaryPrimitives.ReadUInt32BigEndian(span[RecordHeaderSchema.GrupTypeOffset..]),
            Stamp = BinaryPrimitives.ReadUInt32BigEndian(span[RecordHeaderSchema.GrupStampOffset..]),
            Unknown = BinaryPrimitives.ReadUInt32BigEndian(span[RecordHeaderSchema.GrupUnknownOffset..])
        };
    }
    ...
    public static void WriteRecordHeader(Stream stream, MainRecordHeader header)
    {
    ...
    public static long WriteGrupHeader(Stream stream, GroupHeader header)
    {
        ...
        BinaryPrimitives.WriteUInt32LittleEndian(buffer[RecordHeaderSchema.GrupSizeOffset..], header.GroupSize);
        BinaryPrimitives.WriteUInt32LittleEndian(buffer[RecordHeaderSchema.GrupLabelOffset..], header.LabelAsUInt);
        BinaryPrimitives.WriteUInt32LittleEndian(buffer[RecordHeaderSchema.GrupTypeOffset..], (uint)header.GroupType);
