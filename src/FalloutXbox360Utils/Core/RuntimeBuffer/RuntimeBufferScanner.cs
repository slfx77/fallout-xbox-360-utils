...
/// <summary>
///     Binary signature scanning and manager structure walking for runtime buffers.
/// </summary>
internal sealed class RuntimeBufferScanner
{
    ...
    public RuntimeBufferScanner(
        ...
        RuntimeBufferNiTMapReader niTMapReader)
    {
        _ctx = ctx;
    ...
    /// </summary>
    internal void RunBinarySignatureScan(BufferExplorationResult result)
    {
        var binaryGaps = _ctx.Coverage.Gaps
        ...
        foreach (var gap in binaryGaps)
        {
            ...
            _ctx.Accessor.ReadArray(gap.FileOffset, buffer, 0, scanSize);
    ...
    private void WalkLargeStruct(BufferExplorationResult result, ResolvedGlobal global)
    {
        ...
        if (fileOffset == null || fileOffset.Value + readSize > _ctx.FileSize)
        {
            return;
        }

        var buffer = new byte[readSize];
        _ctx.Accessor.ReadArray(fileOffset.Value, buffer, 0, readSize);
        ...
        for (var i = 0; i < readSize; i += 4)
        {
            ...
            else if (ptr >= _ctx.ModuleStart && ptr < _ctx.ModuleEnd)
            {
                vtablePtrs++;
            }
            else if (_ctx.IsValidPointer(ptr))
            {
                validPtrs++;
            }
        }
    ...
    private void WalkModelLoader(BufferExplorationResult result, ResolvedGlobal global)
    {
        ...
        var fileOffset = _ctx.VaToFileOffset(global.PointerValue);
        if (fileOffset == null || fileOffset.Value + 48 > _ctx.FileSize)
        {
            return;
        }

        var structBuf = new byte[48];
        _ctx.Accessor.ReadArray(fileOffset.Value, structBuf, 0, 48);

        // Count valid pointers in the struct
        var validPtrs = _ctx.CountValidPointers(structBuf);

        // Read pLoadedFileMap pointer at offset 36
        var mapVa = BinaryPrimitives.ReadUInt32BigEndian(structBuf.AsSpan(36, 4));
        if (mapVa == 0 || !_ctx.IsValidPointer(mapVa))
        {
            result.ManagerResults.Add(new ManagerWalkResult
            {
                GlobalName = global.Global.Name,
        ...
        var (hashSize, entryCount, strings) = _niTMapReader.WalkNiTMapBaseStrings(mapVa);
    ...
    private void WalkTextureManager(BufferExplorationResult result, ResolvedGlobal global)
    {
        ...
        var fileOffset = _ctx.VaToFileOffset(global.PointerValue);
        if (fileOffset == null || fileOffset.Value + structSize > _ctx.FileSize)
        {
            return;
        }

        var buffer = new byte[structSize];
        _ctx.Accessor.ReadArray(fileOffset.Value, buffer, 0, structSize);
        ...
        {
            var ptr = BinaryPrimitives.ReadUInt32BigEndian(buffer.AsSpan(i, 4));
            if (ptr != 0 && _ctx.IsValidPointer(ptr))
            {
                depthBufs++;
            }
        }
